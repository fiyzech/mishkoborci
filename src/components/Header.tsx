  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-[#0f111a] text-gray-200 border-b border-gray-800 shadow-xl backdrop-blur-md bg-opacity-95">
      <div className="container mx-auto px-4 h-16 flex justify-between items-center">
        ...
        {/* 2. МОБІЛЬНА ПАНЕЛЬ (По центру і справа) - Тільки іконки */}
        <div className="flex items-center gap-3 md:hidden">
          
          {/* Кнопка "Ціни" (щоб швидко глянути прайс) */}
          <a 
            href="#pricing" 
            className="w-9 h-9 flex items-center justify-center rounded-full bg-gray-800 text-white border border-gray-700 active:scale-95 transition-all"
            aria-label="Ціни"
          >
            <FileText size={18} />
          </a>
          ...
          <a 
            ...
            className="w-9 h-9 flex items-center justify-center rounded-full bg-[#2AABEE]/20 text-[#2AABEE] active:scale-95 transition-all"
            aria-label="Telegram"
          >
            <FaTelegram size={20} />
          </a>
          ...
          <a 
            ...
            target="_blank" 
            rel="noopener noreferrer"
            className="w-9 h-9 flex items-center justify-center rounded-full bg-[#7360F2]/20 text-[#7360F2] active:scale-95 transition-all"
            aria-label="Viber"
          >
            <FaViber size={20} />
          </a>

          {/* ТЕЛЕФОН (Головна кнопка) */}
          <a 
            href={`tel:${phoneNumber}`} 
            className="flex items-center justify-center w-10 h-10 rounded-full bg-[#0096FF] text-white shadow-[0_0_15px_rgba(0,150,255,0.4)] animate-pulse hover:animate-none active:scale-95 transition-all"
        ...
        {/* 3. ДЕСКТОПНЕ МЕНЮ (Тільки для комп'ютерів, lg:flex) */}
        ...
        {/* 4. ДЕСКТОП КОНТАКТИ (Справа) */}
        <div className="hidden lg:flex items-center gap-4">
          <div className="w-px h-6 bg-gray-700 mx-2"></div>
          <a href={`https://t.me/+${phoneNumber}`} className="text-[#2AABEE] hover:text-white hover:-translate-y-1 transition-transform">
            <FaTelegram size={24} />
          </a>
